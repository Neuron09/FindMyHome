...
import { unlikeListing } from "../../services/ListingsService";

function LikedListingDetail() {
  ...
  return (
    ...
    <div style={{ display: "flex", justifyContent: "center" }}>
      <Card title={location.state.listing.type + " at " + location.state.listing.address} bordered={false}>
        ...
        <Button onClick={() => applyListing(location.state.listing)} type="primary" style={{ marginTop: "1rem", paddingLeft: "5rem", paddingRight: "5rem" }}>Unlike</Button>
      </Card>
    </div>
  );
}
export default LikedListingDetail;